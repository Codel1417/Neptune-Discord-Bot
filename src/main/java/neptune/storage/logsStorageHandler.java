...
public class logsStorageHandler {
    ...
    public void writeFile(logObject logEntity) throws IOException {
        File file = new File(logsDir + File.separator + logEntity.getGuildID() + File.separator + logEntity.getChannelID() + File.separator + logEntity.getMessageID() +  ".yaml");
    ...
    public logObject readFile(String messageID, String guildID, String channelID) throws IOException {
        File file = new File(logsDir + File.separator + guildID + File.separator + channelID + File.separator + messageID +  ".yaml");
        // Instantiating a new ObjectMapper as a YAMLFactory
        if (!file.exists()){
            return null;
        }
