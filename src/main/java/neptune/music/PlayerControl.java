...
public class PlayerControl extends ListenerAdapter {
  ...
  public void onMessageReceived(MessageReceivedEvent event) {
      ...
      event
          ...
          .sendMessage(
              "```    //Prefix for all commands: .\n"
                  ...
                  + "    // join [name]  - Joins a voice channel that has the provided name\n"
                  + "    // join [id]    - Joins a voice channel based on the provided id.\n"
                  + "    // leave        - Leaves the voice channel that the bot is currently"
                  + " in.\n"
                  + "    // play         - Plays songs from the current queue. Starts playing"
                  + " again if it was previously paused\n"
                  + "    // play [url]   - Adds a new song to the queue and starts playing if it"
                  + " wasn't playing already\n"
                  + "    // pplay        - Adds a playlist to the queue and starts playing if not"
                  + " already playing\n"
                  + "    // pause        - Pauses audio playback\n"
                  + "    // stop         - Completely stops audio playback, skipping the current"
                  + " song.\n"
                  + "    // skip         - Skips the current song, automatically starting the"
                  + " next\n"
                  ...
                  + "    // list         - Lists the songs in the queue\n"
                  + "    // volume [val] - Sets the volume of the MusicPlayer [10 - 100]\n"
                  + "    // restart      - Restarts the current song or restarts the previous song"
                  + " if there is no current song playing.\n"
                  + "    // repeat       - Makes the player repeat the currently playing song\n"
                  + "    // reset        - Completely resets the player, fixing all errors and"
                  + " clearing the queue.```")
          .queue();
    }
    ...
    if (".join".equals(command[0])) {
      ...
      if (command.length == 1) {
        ...
        try {
          int newVolume = Math.max(5, Math.min(100, Integer.parseInt(command[1])));
  ...
  private void loadAndPlay(
      GuildMusicManager mng, final MessageChannel channel, String url, final boolean addPlaylist) {
