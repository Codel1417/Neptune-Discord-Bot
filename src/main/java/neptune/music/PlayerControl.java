...
public class PlayerControl extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if (".join".equals(command[0])) {
            ...
            if (command.length
                    == 1) // It is only the command to start playback (probably after pause)
            {
                if (player.isPaused()) {
            ...
            event.getChannel()
                    .sendMessage(
                            "Playback has been completely stopped and the queue has been cleared.")
                    .queue();
            ...
            if (track != null) {
                event.getChannel()
                        .sendMessage("Restarting track: " + track.getInfo().title)
                        .queue();
            ...
            event.getChannel()
                    .sendMessage(
                            "Player was set to: **"
                                    + (scheduler.isRepeating() ? "repeat" : "not repeat")
                                    + "**")
                    .queue();
            ...
            if (currentTrack != null) {
                ...
                event.getChannel()
                        .sendMessage("The player is not currently playing anything!")
                        .queue();
            ...
            synchronized (queue) {
                if (queue.isEmpty()) {
                    ...
                    sb.append("\n")
                            .append("Total Queue Time Length: ")
                            .append(getTimestamp(queueLength));
